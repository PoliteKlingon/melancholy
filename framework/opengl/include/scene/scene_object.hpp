...
public:
     ...
     */
    SceneObject()
        : SceneObject(Cube(), ModelUBO(), PhongMaterialUBO()) { }
    ...
    SceneObject(const Geometry& geometry, const ModelUBO& model_ubo, const PhongMaterialUBO& material, GLuint texture = 0)
        : geometry(std::make_shared<Geometry>(geometry)), model_ubo(std::make_shared<ModelUBO>(model_ubo)), material(std::make_shared<PhongMaterialUBO>(material)), texture(texture) { }
